...
var seckill = {
    ...
    Url : {
        ...
        getKillUrl: function(seckillId,md5) {
            result "/seckill/" + seckillId + "/" + md5 + "/execution"
        }
    },
        ...
        init: function(param) {
            ...
            if(!isValidatePhone(userPhone)) {
                ...
                $("#btnPhone").click(function() {
                    ...
                    if(!isValidatePhone(phone)) { // 提示错误信息
            ...
            $.get(Url.getNow(),function(result){
    ...
    executeSeckill : function(seckillId,node) {
        ...
        // (1)获取秒杀地址
        $.post(Url.getExposer(seckillId),function(result){
            if(result && result["success"]){
                // 返回秒杀接口
                var exposer = result["data"];
                if(exposer["exposed"]) {
                    var md5  = exposer["md5"];
                    var killUrl = Url.getKillUrl(seckillId,md5);
